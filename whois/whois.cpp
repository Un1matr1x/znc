...
private:
	...
	bool ZNCWhois(CString& sUser) {
		...
		CString rpl311 = ":irc.znc.in 311 " + MyNick + " ?" + Nickname + " " + Ident + " " + VHost + " " + " * :" + RealName;
		CString rpl379 = ":irc.znc.in 379 " + MyNick + " ?" + Username + " :is using modes" +
			...
			(pUser->BounceDCCs() ? " +BounceDCCs" : " -BounceDCCs") + (pUser->DenyLoadMod() ? " +DenyLoadMod" : " -DenyLoadMod") +
			(pUser->MultiClients() ? " +MultiClients" : " -MultiClients") + (pUser->KeepBuffer() ? " +KeepBuffer" : " -KeepBuffer") +
			(!pUser->HasServers() ? " -HasServers":"") + (!pUser->GetIRCConnectEnabled() ? " -IRCConnectEnabled":""); 
		CString rpl671;
...
/*
 * stuff to get: username, nick, ident, host, realname, vhost, altnick, isadmin, channels, vhosts, isircconnected, isircaway, idletime, partyline channels, isuserattached
 * for admins: chg vhost, loadmod, admin, password, server, their ip address, current connections, 
 * more: check http://people.znc.in/~psychon/znc/doc/classCUser.html
 * figure out how to say if the user is on a secure connection - done
 * later on, make it optionally only show the server if args say we can
 * add whowas. maybe we'll add ison as well, to say whether the user is on irc.
 * find some way to say if user is detached. maybe in server description? - done
 * BrianC suggests using a VCString to contain the numeric replies, instead of individual CStrings, but I'm not sure how to do this. Also, the final version will likely not use strings at all, but just directly output the replies, with the 'getters' directly in the putmodule function args.
 */


//we dont need this, but lets keep it around anyhow:
//CString rpl335 = ":irc.znc.in 335 " + MyNick + " ?" + Username + " :is a Bot on this bouncer";
